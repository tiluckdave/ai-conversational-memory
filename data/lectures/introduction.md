...
_Aside_: Annealing is a reference to metallurgy, where hot materials are cooled gradually,
and shows up in sampling and optimization algorithms such as simulated annealing.

_Technical note_: sampling iteratively with a temperature $$T$$ parameter applied
...
$$\underbrace{\ {the}, \ {mouse}, \ {ate}}_\text{prompt} \ {T=0} \underbrace{\ {the}, \ {cheese}}_\text{completion}.$$
...
for example, $$\ {its house}$$ and $$\ {my homework}$$.
